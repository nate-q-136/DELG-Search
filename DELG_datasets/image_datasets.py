...
class ImageFolderCustomV2(Dataset):
    ...
    def __getitem__(self, index: int) -> Tuple[torch.Tensor, int]:
        ...
        class_name = self.paths[
            index
        ].parent.parent.name  # Take the parent of the image path
        class_idx = self.class_to_idx[class_name]
...
if __name__ == "__main__":
    ...
    classes = find_classes(
        "/Volumes/Untitled 2 1/3-CNN-Tensorflow/26-Pytorch/15-image-similarity-search/fish/DELG-Search/feature_extractor/data"
    )
    print(classes)
    dataset = ImageFolderCustomV2(
        "/Volumes/Untitled 2 1/3-CNN-Tensorflow/26-Pytorch/15-image-similarity-search/fish/DELG-Search/feature_extractor/data"
    )
    for i in range(10):
        img, label = dataset[i]
