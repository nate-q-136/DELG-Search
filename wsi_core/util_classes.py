...
class isInContourV1(Contour_Checking_fn):
    ...
    def __call__(self, pt): 
        ...
        y = float(pt[1])
        test_pt = (x, y)
        result = cv2.pointPolygonTest(self.cont, test_pt, False)
...
class isInContourV2(Contour_Checking_fn):
    ...
    def __call__(self, pt): 
        x = float(pt[0]) + self.patch_size / 2.0
        y = float(pt[1]) + self.patch_size / 2.0
        test_pt = (x, y)
        result = cv2.pointPolygonTest(self.cont, test_pt, False)
        return 1 if result >= 0 else 0
...
class isInContourV3_Easy(Contour_Checking_fn):
	...
	def __call__(self, pt): 
		x_center = float(pt[0]) + self.patch_size / 2.0
		y_center = float(pt[1]) + self.patch_size / 2.0
		center = (x_center, y_center)

		if self.shift > 0:
			all_points = [
				(x_center - self.shift, y_center - self.shift),
				(x_center + self.shift, y_center + self.shift),
				(x_center + self.shift, y_center - self.shift),
				(x_center - self.shift, y_center + self.shift)
			]
		...
		for point in all_points:
			result = cv2.pointPolygonTest(self.cont, point, False)
			if result >= 0:
				return 1
		return 0
