...
class WholeSlideImage(object):
	...
	def segmentTissue(self, seg_level=0, sthresh=20, sthresh_up = 255, mthresh=7, close = 0, use_otsu=False, 
							filter_params={'a_t':100}, ref_patch_size=128, exclude_ids=[], keep_ids=[]):
		"""
		...
		def _filter_contours(contours, hierarchy, filter_params):
			...
			all_holes = []
			print("Number of foreground contours: ", len(hierarchy_1))
		...
		# Check if the desired segmentation level exists
		if seg_level >= len(self.level_dim):
			print(f"Segmentation level {seg_level} not available. Using level 0.")
			seg_level = 0
	...
	def process_contour(self, cont, contour_holes, patch_level, save_path, patch_size = 256, step_size = 256,
		contour_fn='basic', use_padding=True, top_left=None, bot_right=None):
		start_x, start_y, w, h = cv2.boundingRect(cont) if cont is not None else (0, 0, self.level_dim[patch_level][0], self.level_dim[patch_level][1])
